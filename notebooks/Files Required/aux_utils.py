""" 
aux_utils.py
    auxiliary routines used in lab 0
"""
import numpy as np
...
def plt_stationary(x_train, y_train):
    ...
    theta0_range = np.array([50-300., 50+300])
    b_space  = np.linspace(*theta0_range, 100)
    ...
    ax[1].set_xlim(w_range) ; ax[1].set_ylim(theta0_range)
...
def plt_contour_wgrad(x, y, hist, ax, theta1_range=[-10, 30, 1], theta0_range=[-20, 20, 1],
                contours = [0.1,50,1000,5000,10000,50000,100000,500000,1000000,5000000,10000000],
                      resolution=1, w_final=12, b_final=1,step=2 ):
    b0,w0 = np.meshgrid(np.arange(*theta0_range),np.arange(*theta1_range))
    z=np.zeros_like(b0)
    ...
    ax.set_xlabel("$\\theta_1$");  ax.set_ylabel("$\\theta_0$")
    ax.set_title('Contour plot of cost $J(\\theta)$, vs $\\theta_1$, $\\theta_0$ with path of gradient descent')
    w = w_final; b=b_final
    ax.hlines(b, ax.get_xlim()[0],w, lw=2, color=dlpurple, ls='dotted', alpha=0.3)
    ax.vlines(w, ax.get_ylim()[0],b, lw=2, color=dlpurple, ls='dotted', alpha=0.3)
...
def plt_cost_vs_theta1(p_hist, J_hist, x_train, y_train):
    ...
    fig.suptitle(f"Cost should always deacrease")
    #===============
    #  First subplot
    #===============
    ...
    ax.plot(w_array, cost, alpha=0.4)  # això estava comentat...
    ax.plot(x,v, c='r')
    ax.set_title("Cost vs $\\theta_1$ ($\\theta_0$ is set to 0)")
    ax.set_ylabel('Cost')
    ...
    tmp_b,tmp_w = np.meshgrid(np.arange(-60, 60, 2),np.arange(-70, 70, 2))
    z=np.zeros_like(tmp_b)
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i][j] = compute_cost(x_train, y_train, tmp_w[i][j], tmp_b[i][j] )
    ...
    ax.set_ylabel('$\\theta_0$', fontsize=16)
    ax.set_zlabel('\ncost', fontsize=16)
    plt.title('Cost vs ($\\theta_1$, $\\theta_0$)')
...
def add_line(dj_dx, x1, y1, d, ax):
    ...
    ax.plot(x, y, '-', c=dldarkred,zorder=10, linewidth = 3)
    xoff = 30 if x1 == 200 else 10
    ax.annotate(r"$\frac{\partial J}{\partial \Theta_1}$ =%d" % dj_dx, fontsize=24,
...
def plt_gradients(x_train,y_train, f_compute_cost, f_compute_gradient):
    ...
    # Print w vs cost to see minimum
    fix_b = 0
    w_array = np.linspace(-200, 300, 50)
    ...
    ax.plot(w_array, cost, linewidth=3, alpha=0.3)
    ax.set_title("Cost vs $\\theta_1$: gradient illustration; $\\theta_0$ set to 0")
    ax.set_ylabel('Cost')
    ax.set_xlabel('$\\theta_1$')

    # plot lines for fixed b=200
    for tmp_w in [-200,40,300]:  
        fix_b = 0
